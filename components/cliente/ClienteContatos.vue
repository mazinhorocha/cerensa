        ...
        >
          ...
          <template #item.actions="{ item, index }">
            <v-icon
              small
              class="mr-2"
              @click="switchAction('update', index, item)"
            >
              mdi-pencil
            </v-icon>
            <v-icon small @click="remove(index)"> mdi-delete </v-icon>
          </template>
        </v-data-table>
      </v-col>
    </v-row>

    <v-dialog v-model="deleteConfirm" persistent max-width="290">
      <v-card>
        <v-card-title class="text-h5"> deseja remover </v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="deleteConfirm = false">
            Cancelar
          </v-btn>
          <v-btn color="green darken-1" text @click="remove()"> Remover </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog v-model="showContactForm" persistent max-width="600px">
      <v-card>
        <v-card-title>
